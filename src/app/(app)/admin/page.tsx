...
export default async function AdminPage(){
  ...
  return <div className="max-w-[1100px] mx-auto pb-8"><div className="mb-7"><span className="text-[11px] font-extrabold tracking-[0.18em] text-white/45">RAWI ADMIN</span><h1 className="text-[34px] md:text-[42px] tracking-[-0.05em] leading-none mt-3">System settings</h1><p className="text-white/45 mt-2">Manage operational settings without changing code or Vercel configuration.</p></div><div className="space-y-5">
    <section className="bg-rawi-panel border border-white/[.07] rounded-[22px] p-5 md:p-6 shadow-sm"><div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3"><div><div className="flex items-center gap-2"><span className="w-9 h-9 rounded-xl bg-rawi-yellow/20 text-rawi-yellow grid place-items-center"><MetricIcon type="health"/></span><h2 className="text-xl font-bold">Infrastructure usage</h2></div><p className="text-sm text-white/45 mt-2">Monitor RAWI storage and estimated client download traffic before infrastructure costs become a problem.</p></div><span className="self-start rounded-full bg-white/[.08] border border-white/[.10] px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-wider text-white/60">{billingWorkspace.plan||"free"} plan</span></div><div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-5"><UsageMetric icon="storage" label="Storage used" value={fmtGb(used)} detail={limit?`of ${fmtGb(limit)}`:""}/><UsageMetric icon="download" label="Downloads · 30d" value={String(downloadCount30d)} detail={`${fmtGb(downloadBytes30d)} estimated traffic`}/><UsageMetric icon="cost" label="Cached estimate" value={`AED ${estimatedCachedCostAed.toFixed(2)}`} detail="at $0.03 / GB"/><UsageMetric icon="cost" label="Origin estimate" value={`AED ${estimatedOriginCostAed.toFixed(2)}`} detail="at $0.09 / GB"/></div><div className="mt-5"><div className="flex justify-between text-xs font-bold mb-2"><span>Workspace storage</span><span>{storagePct.toFixed(1)}%</span></div><div className="h-2 rounded-full bg-white/[.08] overflow-hidden"><div className="h-full rounded-full bg-rawi-yellow" style={{width:`${storagePct}%`}}/></div></div><p className="mt-4 text-[11px] leading-5 text-white/45">Download traffic is an application estimate based on original media file size × recorded downloads during the last 30 days. Actual Supabase billed egress can differ because previews, thumbnails, video streaming, cache hits and other RAWI services also use bandwidth.</p></section>

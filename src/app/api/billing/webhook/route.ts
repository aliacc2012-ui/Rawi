...
async function updateWorkspacePlan(workspaceId: string, plan: "free" | "creator" | "pro") {
  ...
  await admin
    .from("workspaces")
    .update({ plan, storage_limit_bytes: PLAN_STORAGE[plan] })
    .eq("id", workspaceId);
}
...
export async function POST(request: NextRequest) {
  ...
  if (workspaceId) {
    if (event.type === "checkout.session.completed") {
      if (requestedPlan === "creator" || requestedPlan === "pro") {
        await updateWorkspacePlan(workspaceId, requestedPlan);
      }
    }

    if (event.type === "customer.subscription.updated") {
      const active = object?.status === "active" || object?.status === "trialing";
      if (active && (requestedPlan === "creator" || requestedPlan === "pro")) {
        await updateWorkspacePlan(workspaceId, requestedPlan);
      }
    }

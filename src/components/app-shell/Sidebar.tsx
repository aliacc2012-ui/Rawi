...
export function Sidebar({ storageUsedBytes, storageLimitBytes }: { storageUsedBytes: number; storageLimitBytes: number }) {
  ...
  return <>
    <aside className="hidden md:flex bg-rawi-ink text-white p-5 flex-col w-[230px] shrink-0 min-h-screen sticky top-0 h-screen">
      ...
      <nav className="flex flex-col gap-2 mt-10">{LINKS.map((link) => { const active = pathname.startsWith(link.href); const Icon = link.Icon; return <Link key={link.href} href={link.href} className={`relative flex items-center gap-3 px-3.5 py-3 rounded-xl font-bold transition ${active ? "bg-[#1d1d1d] text-white before:absolute before:-left-5 before:w-[3px] before:h-7 before:rounded-full before:bg-rawi-yellow" : "text-white/45 hover:bg-[#1d1d1d] hover:text-white"}`}><Icon className={`h-[19px] w-[19px] shrink-0 ${active ? "text-rawi-yellow" : "text-white/40"}`} />{link.label}</Link>; })}</nav>
      <div className="mt-auto"><div className="text-[10px] text-white/45 flex justify-between mb-2"><span>Storage</span><span>{gb(storageUsedBytes)} / {gb(storageLimitBytes)} GB</span></div><div className="h-1.5 bg-[#292929] rounded-full overflow-hidden mb-4"><div className="h-full bg-rawi-yellow" style={{ width: `${pct}%` }} /></div><Link href="/settings" className="flex items-center gap-2 text-xs text-white/60 py-3 border-t border-white/10"><CrownIcon className="h-4 w-4 text-rawi-yellow" /> Upgrade plan</Link><Link href="/support" className="mb-3 block text-center text-xs font-bold text-white/45 hover:text-white">Help & support</Link><form action={signOut}><button type="submit" className="w-full text-white/60 border border-[#333] rounded-full px-4 py-2.5 text-sm hover:bg-[#1d1d1d]">Sign out</button></form></div>
    </aside>

    <div className="md:hidden sticky top-0 z-40 -mx-4 -mt-4 mb-4 flex items-center justify-between border-b border-black/5 bg-[#06060F]/95 px-4 py-3 backdrop-blur-xl sm:-mx-5 sm:-mt-5 sm:px-5">
      ...
      <div className="flex items-center gap-2"><Link href="/support" className="rounded-full px-2 py-2 text-xs font-bold text-white/50">Help</Link><form action={signOut}><button type="submit" className="rounded-full border border-black/10 bg-rawi-panel px-3.5 py-2 text-xs font-extrabold shadow-sm active:scale-[.98]">Sign out</button></form></div>
    </div>
    ...
    <nav className="md:hidden fixed inset-x-3 bottom-3 z-50 grid grid-cols-5 rounded-2xl border border-white/10 bg-[#111]/95 p-1.5 text-white shadow-2xl backdrop-blur-xl" aria-label="Mobile navigation">
      {LINKS.map((link) => { const active = pathname.startsWith(link.href); const Icon = link.Icon; return <Link key={link.href} href={link.href} className={`flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-2 text-[10px] font-bold transition ${active ? "bg-rawi-panel/10 text-white" : "text-white/45"}`}><Icon className={`h-[18px] w-[18px] ${active ? "text-rawi-yellow" : ""}`} /><span className="truncate">{link.label}</span></Link>; })}
    </nav>
  </>;
}

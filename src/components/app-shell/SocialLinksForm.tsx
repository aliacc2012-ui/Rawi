...
export function SocialLinksForm({ workspaceId, initialLinks }: { workspaceId: string; initialLinks: Links }) {
  ...
  return <div className="mt-5"><div className="rounded-2xl border border-white/[.07] overflow-hidden bg-rawi-panel">{items.map((item,index)=>{const url=normalizeUrl(links[item.key]);return <div key={item.key} className={`grid grid-cols-1 sm:grid-cols-[46px_100px_minmax(0,1fr)_auto_auto] items-center gap-3 p-3.5 ${index!==items.length-1?"border-b border-white/[.06]":""}`}><div className="w-11 h-11 rounded-xl bg-rawi-panel grid place-items-center">{item.icon}</div><div className="text-sm font-extrabold">{item.name}</div><input type="url" value={links[item.key]} onChange={(e)=>{setLinks(p=>({...p,[item.key]:e.target.value}));setMessage("");}} placeholder={item.placeholder} className="min-w-0 w-full rounded-xl border border-white/[.07] bg-rawi-panel/[.04] px-3.5 py-2.5 text-sm outline-none focus:border-black"/>{url?<a href={url} target="_blank" rel="noreferrer" className="rounded-xl border border-white/[.07] px-3.5 py-2.5 text-xs font-extrabold">Visit ↗</a>:<span/>}<button type="button" onClick={()=>{setLinks(p=>({...p,[item.key]:""}));setMessage("");}} className="w-9 h-9 rounded-xl border border-white/[.07] text-white/45">×</button></div>})}</div><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-4"><div className="text-xs text-white/45">{connectedCount} of {items.length} profiles connected</div><div className="flex items-center gap-3">{message&&<span className={`text-xs font-bold ${message.includes("✓")?"text-emerald-600":"text-red-600"}`}>{message}</span>}<button type="button" disabled={pending} onClick={save} className="rounded-xl bg-black text-white px-5 py-3 text-xs font-extrabold disabled:opacity-50">{pending?"Saving…":"Save social links"}</button></div></div><p className="text-[10px] text-white/45 mt-3">Saved to your RAWI workspace, so these links follow you across devices.</p></div>;
}

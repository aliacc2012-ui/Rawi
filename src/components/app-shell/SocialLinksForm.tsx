...
function normalizeUrl(value: string) { const clean=value.trim(); return clean && !/^https?:\/\//i.test(clean) ? `https://${clean}` : clean; }
...
export function SocialLinksForm({ workspaceId, initialLinks }: { workspaceId: string; initialLinks: Links }) {
  ...
  function save() { setMessage(""); startTransition(async()=>{ const normalized=Object.fromEntries(Object.entries(links).map(([k,v])=>[k,normalizeUrl(v)])) as Links; const result=await updateSocialLinks(workspaceId, normalized); if(result?.error){setMessage(result.error);return;} setLinks(normalized); setMessage("Saved ✓"); }); }

  ...
  return (
    <section className="mx-auto grid min-h-0 w-[min(1240px,calc(100%-32px))] items-center gap-10 overflow-hidden pb-14 pt-8 md:min-h-[760px] md:w-[min(1240px,calc(100%-40px))] md:grid-cols-[.92fr_1.08fr] md:gap-[58px] md:pb-[80px] md:pt-10">
      ...
      <style jsx global>{`
        ...
        @keyframes rawi-character-sequence {
          0%, 6% { opacity: 0; transform: translate3d(145%, 8px, 0); }
          10% { opacity: 1; transform: translate3d(105%, 0, 0); }
          29% { opacity: 1; transform: translate3d(0, 0, 0); }
          48% { opacity: 1; transform: translate3d(0, 0, 0); }
          72% { opacity: 1; transform: translate3d(-150%, 2px, 0); }
          76%, 100% { opacity: 0; transform: translate3d(-165%, 8px, 0); }
        }
        @keyframes rawi-walk-bob {
          0%, 100% { transform: translateY(0) rotate(.35deg); }
          50% { transform: translateY(-4px) rotate(-.35deg); }
        }
        @keyframes rawi-walk-pose {
          0%, 27% { opacity: 1; }
          30%, 100% { opacity: 0; }
        }
        @keyframes rawi-wave-pose {
          0%, 28% { opacity: 0; }
          31%, 49% { opacity: 1; }
          52%, 100% { opacity: 0; }
        }
        @keyframes rawi-exit-pose {
          0%, 49% { opacity: 0; }
          52%, 100% { opacity: 1; }
        }
        @keyframes rawi-wave-hello {
        ...
        .rawi-character { animation: rawi-character-sequence 20s 2s cubic-bezier(.45,0,.2,1) infinite; will-change: transform, opacity; aspect-ratio: 2 / 3; }
        .rawi-pose { opacity: 0; }
        .rawi-pose-walk { animation: rawi-walk-pose 20s 2s linear infinite, rawi-walk-bob .7s ease-in-out infinite; }
        .rawi-pose-wave { animation: rawi-wave-pose 20s 2s linear infinite, rawi-wave-hello 1.2s ease-in-out infinite; }
        .rawi-pose-exit { animation: rawi-exit-pose 20s 2s linear infinite, rawi-walk-bob .7s ease-in-out infinite reverse; }
        @media (prefers-reduced-motion: reduce) {
          .rawi-dashboard-enter, .rawi-phone-enter, .rawi-delivery-enter, .rawi-mockup-glow, .rawi-character, .rawi-pose { animation: none !important; } .rawi-pose-walk { opacity: 1; }
        }
      `}</style>
    </section>
  );

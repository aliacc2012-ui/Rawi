    ...
    <section className="mx-auto grid min-h-0 w-[min(1240px,calc(100%-32px))] items-center gap-10 overflow-hidden pb-14 pt-8 md:min-h-[760px] md:w-[min(1240px,calc(100%-40px))] md:grid-cols-[.92fr_1.08fr] md:gap-[58px] md:pb-[80px] md:pt-10">
      <div>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-5 md:gap-7">
          <Link href="/signup" onClick={() => trackEvent("signup_started", { source: "hero" })} className="rounded-xl bg-rawi-yellow px-6 py-4 text-sm font-extrabold shadow-[0_12px_30px_rgba(255,200,0,.22)] md:px-7 md:text-base">{dict.hero.startFree}</Link>
          <a href="/demo/today-drive" className="border-b border-black pb-1 text-sm font-bold md:text-base">{dict.hero.viewDemo} <span>↘</span></a>
        </div>
      ...
      <style jsx global>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .rawi-dashboard-enter, .rawi-delivery-enter, .rawi-mockup-glow { animation: none !important; }
        }
      `}</style>
    </section>
  );
    ...
    <section className="bg-[#fbf6ef] px-4 pb-10 pt-2 md:px-5 md:pb-16 md:pt-5">
      <div className="relative mx-auto w-full max-w-[1380px] overflow-hidden rounded-[26px] md:rounded-[32px] border border-[#e4dbcf] bg-white/75 px-5 py-8 md:px-10 md:py-12 shadow-[0_24px_80px_rgba(55,42,24,.07)] backdrop-blur-sm md:px-10 md:py-12">
        ...
        <div className="relative z-10 grid items-center gap-10 xl:grid-cols-[.88fr_1.35fr_auto]">
          ...
          <Link href="/signup" onClick={() => trackEvent("signup_started", { source: "closing_cta" })} className="group inline-flex w-full shrink-0 items-center justify-center gap-3 xl:w-auto rounded-full bg-black px-6 py-4 text-sm font-extrabold text-white shadow-[0_14px_34px_rgba(0,0,0,.18)] transition-transform hover:-translate-y-0.5">
            <span className="h-2 w-2 rounded-full bg-rawi-yellow" />

  ...
  return (
    <section className="mx-auto grid min-h-0 w-[min(1240px,calc(100%-32px))] items-center gap-10 overflow-hidden pb-14 pt-8 md:min-h-[760px] md:w-[min(1240px,calc(100%-40px))] md:grid-cols-[.92fr_1.08fr] md:gap-[58px] md:pb-[80px] md:pt-10">
      ...
      <style jsx global>{`
        ...
        @keyframes rawi-character-walk {
          0%, 7% { opacity: 0; transform: translate3d(145%, 10px, 0) rotate(2deg); }
          18% { opacity: 1; transform: translate3d(48%, 0, 0) rotate(0deg); }
          28%, 66% { opacity: 1; transform: translate3d(0, 0, 0) rotate(0deg); }
          76% { opacity: 1; transform: translate3d(-50%, 0, 0) rotate(-1deg); }
          88%, 100% { opacity: 0; transform: translate3d(-150%, 8px, 0) rotate(-2deg); }
        }
        @keyframes rawi-character-step {
          0%, 100% { margin-top: 0; }
          50% { margin-top: -5px; }
        }
        @keyframes rawi-glow {
        ...
        .rawi-character { animation: rawi-character-walk 15s 1.8s cubic-bezier(.45,0,.2,1) infinite; will-change: transform, opacity; }
        .rawi-character img { animation: rawi-character-step .72s ease-in-out infinite; }
        @media (prefers-reduced-motion: reduce) {
          .rawi-dashboard-enter, .rawi-phone-enter, .rawi-delivery-enter, .rawi-mockup-glow, .rawi-character, .rawi-character img { animation: none !important; }
        }
      `}</style>
    </section>
  );

...
export function EnhancedFeatures() {
  return (
    <section id="features" className="bg-[#0e0e0e] pt-[72px] pb-0 md:pt-[110px]">
      <div className="w-[min(1180px,calc(100%-32px))] md:w-[min(1180px,calc(100%-40px))] mx-auto">
        <div className="grid lg:grid-cols-[.72fr_1.28fr] gap-10 lg:gap-14 items-end mb-10">
          <div>
            <div className="text-[11px] font-extrabold tracking-[0.17em] text-white/35">BUILT AROUND DELIVERY</div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
          {SUPPORTING_FEATURES.map((feature) => (
            <article
              key={feature.number}
              className="group rounded-[22px] border border-white/8 bg-[#161616] p-5 sm:p-6 min-h-0 sm:min-h-[255px] shadow-[0_10px_35px_rgba(0,0,0,.2)] transition duration-300 hover:-translate-y-1 hover:border-white/15 hover:shadow-[0_18px_45px_rgba(0,0,0,.4)]"
            >
              ...
              <h3 className="text-[22px] leading-[1.08] tracking-[-.035em] font-bold mt-6 text-[#f5f5f0]">{feature.title}</h3>
      ...
      <div className="relative left-1/2 mt-14 md:mt-20 w-screen -translate-x-1/2 overflow-hidden border-y border-white/8 bg-[#141414] py-7 md:py-9">
        ...
        <div className="pointer-events-none absolute inset-y-0 left-0 w-20 bg-gradient-to-r from-[#141414] to-transparent md:w-40" />
        <div className="pointer-events-none absolute inset-y-0 right-0 w-20 bg-gradient-to-l from-[#141414] to-transparent md:w-40" />
        <style>{`
...
function HeroFeature({ number, eyebrow, title, body, badges, visual }: { number: string; eyebrow: string; title: React.ReactNode; body: string; badges: string[]; visual: React.ReactNode }) {
  return (
    <article className="group relative overflow-hidden rounded-[24px] md:rounded-[28px] border border-white/8 bg-[#161616] min-h-0 p-5 sm:p-7 md:min-h-[360px] md:p-8 shadow-[0_12px_40px_rgba(0,0,0,.3)] transition duration-300 hover:-translate-y-1 hover:border-white/15 hover:shadow-[0_22px_55px_rgba(0,0,0,.5)]">
...
function MiniBenefit({ icon, title, body }: { icon: string; title: string; body: string }) {
  return (
    <div className="border-l border-white/10 first:border-l-0 first:pl-0 pl-2 sm:pl-4">
      ...
      <div className="text-xs font-extrabold mt-2 text-[#f5f5f0]">{title}</div>

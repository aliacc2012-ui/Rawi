      ...
      {viewerOpen && url && (
        ...
        >
            ...
            <div className="flex items-center gap-2" onClick={(event) => event.stopPropagation()}>
              {mediaType === "image" && (
                <>
                  <button
                    type="button"
                    onClick={zoomOut}
                    className="h-9 min-w-9 px-3 rounded-full bg-white/10 text-white hover:bg-white/20"
                    aria-label="Zoom out"
                    title="Zoom out"
                  >
                    −
                  ...
                  <button
                    type="button"
                    onClick={resetZoom}
                    className="h-9 px-3 rounded-full bg-white/10 text-white text-xs hover:bg-white/20"
              ...
              {downloadsEnabled && (
                <button
                  type="button"
                  onClick={handleDownload}
                  className="h-9 px-4 rounded-full bg-white/10 text-white text-xs hover:bg-white/20"
                >
                  Download
                </button>
              )}
              <button
                type="button"
                onClick={closeViewer}
                className="h-9 min-w-9 px-3 rounded-full bg-white text-black font-bold"
          ...
          <div className="flex-1 overflow-auto grid place-items-center p-4 md:p-8" onClick={(event) => event.stopPropagation()}>
            {mediaType === "video" ? (
              <video src={url} controls autoPlay className="max-w-full max-h-[calc(100vh-7rem)]" />
            ) : (
              // eslint-disable-next-line @next/next/no-img-element
              <img
                src={url}
                ...
                className="max-w-full max-h-[calc(100vh-7rem)] object-contain transition-transform duration-150 origin-center select-none"
              />
            )}
          </div>
        </div>
